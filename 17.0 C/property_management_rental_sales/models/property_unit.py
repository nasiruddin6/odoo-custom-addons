...
class PropertyUnit(models.Model):
    ...
    owner_mobile = fields.Char(related='owner_partner_id.mobile', string='Owner Mobile', store=True)
    ...
    def unlink(self):
        if self.state != 'draft':
            raise UserError(_("You can't delete property that's not in 'Draft'!"))

        if self.contract_count + self.sales_contract_count >= 1:
            raise UserError(_("You can't delete contracts that has contract!"))

        return super(PropertyUnit, self).unlink()

    @api.depends('property_for')
    def _get_partner_domain(self):
        for rec in self:
            domain = []
            if rec.property_for == 'rent':
    ...
    def _onchange_property_id_taxes(self):
        """Inherit taxes from property when property_id changes."""
        if self.property_id and self.property_id.taxes_id:
            self.taxes_id = self.property_id.taxes_id
    ...
    def action_view_contracts(self):
        self.ensure_one()
        contracts = self.env['property.contract'].sudo().search([
            ('unit_id', '=', self.id)
        ])
        action = {
            'name': 'Contracts',
    ...
    def action_view_sales_contracts(self):
        ...
        if self.sales_contract_count == 1:
            sale_id = self.env['sale.order'].sudo().search([
                ('unit_id', '=', self.id)
            ], limit=1)
            action.update({
                'view_mode': 'form',
                'res_id': sale_id.id if sale_id else False
            })
    ...
    def create_rent_contract(self):
        ...
        contract_vals = {
            ...
            'tax_ids': [(6, 0, self.taxes_id.ids)] if self.taxes_id else False,
        }
    ...
    def create_sell_contract(self):
        ...
        order_line = [(0, 0, {
            ...
            'product_uom_qty': 1,
            'price_unit': price,
            'tax_id': [(6, 0, self.taxes_id.ids)] if self.taxes_id else False,
        })]
