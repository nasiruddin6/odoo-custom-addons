...
class ContractSettlementWizard(models.TransientModel):
    ...
    def action_settlement(self):
        ...
        if self.type == 'receive':
            ...
            total_unpaid_amt = sum(unpaid_invoices.mapped('amount_residual'))
            if total_unpaid_amt < self.amount:
                amt_due = self.amount - total_unpaid_amt
                inv = self._action_create_invoice(amt_due)
                inv.action_post()
                unpaid_invoices |= inv
        ...
        self.settlement_id.write(vals)
        self.settlement_id.contract_id.state = 'settled'
        self.settlement_id.contract_id.unit_id.state = 'available'

    def _action_create_invoice(self, amount):
        tax_ids = []
        if amount == 0:
            return
