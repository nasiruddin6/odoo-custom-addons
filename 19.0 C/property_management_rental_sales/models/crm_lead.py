...
class CrmLead(models.Model):
    ...
    visit_id = fields.Many2one('crm.property.visit', 'Visit', tracking=True)
    visit_state = fields.Selection(related='visit_id.state', string='Visit State', tracking=True)
    ...
    def action_schedule_visit(self):
        """
        if no visit scheduled than
        return a form view else show previous one
        """
        self.ensure_one()
        # Return visiting form view to create visiting
        if self.visit_id:
            return self.action_view_visits()
